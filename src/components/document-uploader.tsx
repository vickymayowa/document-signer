        ...
        <div>
          ...
          >
            ...
            <div className="flex flex-col items-center justify-center gap-4">
              <div className={`rounded-full p-4 ${isDragActive && isDragAccept ? "bg-primary/10" : "bg-muted"}`}>
                <Upload
                  className={`h-10 w-10 ${isDragActive && isDragAccept ? "text-primary" : "text-muted-foreground"}`}
                />
              </div>
              <div>
                <h3 className="text-xl font-medium mb-2">Drag & drop your PDF here</h3>
        ...
        <div className="border rounded-lg p-6">
          <div className="flex items-start gap-4 mb-6">
            ...
            <div className="flex-1">
              ...
              {isUploading ? (
                <p className="text-sm text-amber-500 mt-1">Processing document...</p>
              ) : (
          ...
          {isUploading ? (
            ...
            <div className="bg-muted/50 rounded-lg p-4 mb-6">
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                {metadata.title && (
                  <div>
                    <span className="text-muted-foreground">Title:</span> {metadata.title}
                  </div>
                )}
                {metadata.author && (
                  <div>
                    <span className="text-muted-foreground">Author:</span> {metadata.author}
                  </div>
                )}
                ...
                {metadata.pageCount && (
                  <div>
                    <span className="text-muted-foreground">Pages:</span> {metadata.pageCount}
                  </div>
                )}
              </div>
            </div>
